...
"""
Text utilities for LazyGraphRAG.

This module contains text processing utilities, extracted from GraphRAG
...
def try_parse_json_object(input: str, verbose: bool = True) -> tuple[str, dict]:
    ...
    Sometimes, the LLM returns a json string with some extra description,
    this function will clean it up.

    Args:
        input: The raw string potentially containing JSON.
        verbose: Whether to log warnings on parse failures.

    Returns:
        Tuple of (cleaned_string, parsed_dict)
    """
    result = None
    try:
